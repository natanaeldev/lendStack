...
function ThresholdForm({
  ...
  return (
    <div className="rounded-[28px] border border-blue-100 bg-white p-5 shadow-sm space-y-4">
...
function ApprovalForm({
  ...
  async function save() {
    ...
    try {
      ...
      const res    = await fetch(url, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name, minAmount: Number(minAmount),
          maxAmount: maxAmount ? Number(maxAmount) : null,
          currency, approvalMode,
          requiredApprovalCount: Number(requiredCount),
          rejectionMode, biometricMode,
  ...
  return (
    <div className="rounded-[28px] border border-violet-100 bg-white p-5 shadow-sm space-y-4">
...
export default function ReauthPoliciesPage() {
  ...
  const TAB_CLS = (active: boolean) =>
    `px-4 py-2 rounded-full text-sm font-bold transition-colors ${active ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-900'}`
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-6 sm:px-8">
      ...
      <div className="mt-6 flex gap-2">
        <button onClick={() => setTab('threshold')} className={TAB_CLS(tab === 'threshold')}>Umbrales</button>
        <button onClick={() => setTab('approval')}  className={TAB_CLS(tab === 'approval')}>Aprobaciones</button>
        <button onClick={() => setTab('logs')}      className={TAB_CLS(tab === 'logs')}>Historial</button>
      </div>
                  ...
                  <div key={t._id} className="flex items-center justify-between rounded-2xl border border-slate-100 bg-white px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Umbral: {new Intl.NumberFormat('es-DO', { style: 'currency', currency: t.currency }).format(t.thresholdAmount)}
                        {' '}&middot; {t.currency}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          ...
                          {' '}&middot; {APPROVAL_MODE_LABELS[p.approvalMode]}
                          {' '}&middot; {BIOMETRIC_MODE_LABELS[p.biometricMode]}
                          {' '}&middot; {p.approvers.length} aprobador(es)
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
              ...
              <div className="space-y-2">
                {logs.map((log: any) => (
                  <div key={log._id} className="rounded-2xl border border-slate-100 bg-white px-4 py-3 text-sm">
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-slate-800 capitalize">{log.policyType} política</span>
                      <span className="text-xs text-slate-400">
                        {new Date(log.createdAt).toLocaleString('es-DO')}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500">ID: {log.policyId} &middot; Por: {log.changedBy}</p>
                  </div>
                ))}
              </div>
            )}
