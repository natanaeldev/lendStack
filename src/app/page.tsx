...
import {
  ...
  RiskProfile, Currency, LoanParams,
  formatCurrency,
} from '@/lib/loan'
...
export default function Home() {
  ...
  const params: LoanParams = { amount, termYears, profile, currency }
        ...
        {tab === 'calculator' && (
          <>
            ...
            <div className={card} style={cardShadow}>
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">Perfil de riesgo</label>
                <RiskSelector value={profile} onChange={setProfile} />
              </div>
            </div>

            {/* Results */}
            <div className="mb-5 fade-up-1">
              <ResultsPanel result={result} config={config} currency={currency} />
            </div>
