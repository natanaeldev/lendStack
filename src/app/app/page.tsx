            ...
            {isMaster && (
              <>
                ...
                <Link
                  href="/admin/users"
                  onClick={() => setMobileNavOpen(false)}
                  className="mt-1 w-full rounded-xl px-3 py-3 text-left transition-all border border-transparent hover:border-amber-200 hover:bg-amber-50 flex items-center gap-2.5"
                  style={{ color: '#92400E' }}
                >
                  <span className="text-lg leading-none">👥</span>
                  <div>
                    <p className="text-sm font-semibold">Registrar usuario</p>
                    <p className="text-xs text-slate-400">Alta y gestión de usuarios</p>
                  </div>
                </Link>
                <Link
                  href="/admin/branches"
                  onClick={() => setMobileNavOpen(false)}
                  className="w-full rounded-xl px-3 py-3 text-left transition-all border border-transparent hover:border-sky-200 hover:bg-sky-50 flex items-center gap-2.5"
                  style={{ color: '#0369A1' }}
                >
                  <span className="text-lg leading-none">🏢</span>
                  <div>
                    <p className="text-sm font-semibold">Registrar sucursal</p>
                    <p className="text-xs text-slate-400">Alta y configuración de sucursales</p>
                  </div>
                </Link>
              </>
            )}
      ...
      <aside className={`hidden lg:flex fixed left-0 top-[88px] bottom-0 z-30 border-r border-slate-200 bg-white/95 backdrop-blur flex-col p-3 gap-2 transition-all duration-300 ${sidebarCollapsed ? 'w-20' : 'w-64'}`}
        ...
        <nav className="space-y-1" aria-label="Navegación principal">
          ...
          {isMaster && (
            <>
              <div className="my-2 border-t border-slate-200" />
              {!sidebarCollapsed && <p className="px-2 text-[11px] font-bold uppercase tracking-wider text-slate-400">Administración</p>}
              <Link
                href="/admin/users"
                className="w-full text-left rounded-xl transition-all border border-transparent hover:border-amber-200 hover:bg-amber-50"
                style={{ color: '#92400E', padding: sidebarCollapsed ? '10px' : '12px' }}
                title={sidebarCollapsed ? 'Registrar usuario' : undefined}
              >
                <div className="flex items-center gap-2.5">
                  <span className="text-lg leading-none">👥</span>
                  {!sidebarCollapsed && (
                    <div>
                      <p className="text-sm font-semibold">Registrar usuario</p>
                      <p className="text-xs text-slate-400 mt-0.5">Alta y gestión de usuarios</p>
                    </div>
                  )}
                </div>
              </Link>
              <Link
                href="/admin/branches"
                className="w-full text-left rounded-xl transition-all border border-transparent hover:border-sky-200 hover:bg-sky-50"
                style={{ color: '#0369A1', padding: sidebarCollapsed ? '10px' : '12px' }}
                title={sidebarCollapsed ? 'Registrar sucursal' : undefined}
              >
                <div className="flex items-center gap-2.5">
                  <span className="text-lg leading-none">🏢</span>
                  {!sidebarCollapsed && (
                    <div>
                      <p className="text-sm font-semibold">Registrar sucursal</p>
                      <p className="text-xs text-slate-400 mt-0.5">Alta y configuración de sucursales</p>
                    </div>
                  )}
                </div>
              </Link>
