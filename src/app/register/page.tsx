...
export default function RegisterPage() {
  ...
  const handleNext = (e: React.FormEvent) => {
    ...
    if (!orgName.trim()) { setError('Ingresá el nombre de tu organización.'); return }
    if (!adminEmail.trim()) { setError('El email es obligatorio.'); return }
            ...
            {[1, 2].map(s => (
              <div key={s} className="flex items-center gap-2">
                <div
                  ...
                  style={{
                    background: step >= s ? '#1565C0' : 'rgba(255,255,255,.2)',
                    color: step >= s ? '#fff' : 'rgba(255,255,255,.5)',
                  }}
                >
                  {s}
                </div>
                {s < 2 && (
                  <div
                    ...
                    style={{ background: step > s ? '#1565C0' : 'rgba(255,255,255,.2)' }}
                  />
                )}
              </div>
            ))}
          ...
          <div className="bg-white rounded-2xl p-8" style={{ boxShadow: '0 8px 48px rgba(0,0,0,.35)' }}>
            ...
            {step === 1 && (
              <>
                ...
                <p className="text-sm text-slate-400 mb-6">
                  Registrá tu empresa en LendStack — es gratis para empezar.
                </p>
                ...
                <form onSubmit={handleNext} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Ej: Créditos Rápidos SRL" autoComplete="organization"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="admin@tuempresa.com" required autoComplete="email"
                      className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 text-sm focus:outline-none focus:border-blue-500 transition-colors"
                      style={{ color: '#374151' }}
                    />
                  </div>
            ...
            {step === 2 && (
              <>
                ...
                <p className="text-sm text-slate-400 mb-6">
                  Podés empezar gratis y actualizar cuando quieras.
                </p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-end gap-1 mb-3">
                          <span className="text-xl font-black" style={{ color: '#0D2B5E' }}>{pd.price}</span>
                          <span className="text-xs text-slate-400 pb-0.5">{pd.priceNote}</span>
                        </div>
                        <p className="text-sm font-bold mb-2" style={{ color: '#0D2B5E' }}>{pd.label}</p>
                ...
                <p className="text-center text-xs text-slate-400 mt-4">
                  ...
                  <span className="underline cursor-pointer" style={{ color: '#1565C0' }}>Términos de uso</span>
                  {' '}y la{' '}
                  <span className="underline cursor-pointer" style={{ color: '#1565C0' }}>Política de privacidad</span>.
                </p>
              </>
            )}
          </div>
