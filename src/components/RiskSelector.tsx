'use client'
import { RISK_PROFILES, RiskProfile, RateMode } from '@/lib/loan'
...
export default function RiskSelector({ value, onChange, rateMode = 'annual' }: Props) {
      ...
      {RISK_PROFILES.map((p) => {
        ...
        return (
          ...
          >
            ...
            {isMonthly ? (
              /* In monthly mode: show the annual reference rates dimmed, for context only */
              <div className="text-xs mb-1" style={{ color: p.colorAccent }}>
                <span className="font-bold">{(p.minRate / 12 * 100).toFixed(2)}%–{(p.maxRate / 12 * 100).toFixed(2)}%</span>
                <span className="ml-1 font-normal text-slate-400">mensual ref.</span>
              </div>
            ) : (
