...
function PolicyForm({
  ...
  return (
    <div className="space-y-5 rounded-[28px] border border-indigo-100 bg-white p-5 shadow-sm">
      ...
      <div className="flex items-center gap-6">
        <label className="cursor-pointer text-sm text-slate-700 flex items-center gap-2">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-2xl border border-green-100 bg-green-50 p-4">
          <p className="text-xs font-bold uppercase tracking-wider text-green-700">Aprobación automática (todos deben cumplirse)</p>
        ...
        <div className="space-y-3 rounded-2xl border border-amber-100 bg-amber-50 p-4">
          <p className="text-xs font-bold uppercase tracking-wider text-amber-700">Revisión manual (todos deben cumplirse)</p>
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              <label className={labelCls}>Puntuación crédito ≥</label>
        ...
        <div className="space-y-3 rounded-2xl border border-red-100 bg-red-50 p-4">
          <p className="text-xs font-bold uppercase tracking-wider text-red-700">Rechazo automático (cualquiera activa el rechazo)</p>
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              <label className={labelCls}>Puntuación crédito &lt;</label>
...
export default function LoanPolicyContent() {
  ...
  return (
    <div className="bg-slate-50">
      ...
      <div className="mx-auto max-w-3xl space-y-4 px-4 py-6">
        ...
        {showForm && <PolicyForm existing={editing} onSaved={afterSave} onCancel={cancel} />}
        ...
        {policies.map((p) => (
          <div
            key={p._id}
            className={`space-y-3 rounded-[28px] border bg-white p-5 shadow-sm ${p.is_active ? 'border-indigo-100' : 'border-slate-100 opacity-60'}`}
          >
            <div className="flex items-start justify-between gap-2">
              ...
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${p.is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-slate-600 sm:grid-cols-4">
              <div className="rounded-xl bg-slate-50 p-2">
                <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Crédito mín.</p>
                <p className="mt-0.5 font-semibold">{p.min_credit_score}</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-2">
                <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Deuda/ingreso máx.</p>
                <p className="mt-0.5 font-semibold">{p.max_debt_to_income_ratio}%</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-2">
                <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Ingreso mín.</p>
                <p className="mt-0.5 font-semibold">{p.min_monthly_income.toLocaleString()}</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-2">
                <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Préstamo máx.</p>
                <p className="mt-0.5 font-semibold">{p.max_loan_amount.toLocaleString()}</p>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="rounded-xl border border-green-100 bg-green-50 p-2 text-green-800">
                <p className="mb-1 text-[10px] font-bold uppercase tracking-wide">Auto-aprobación</p>
              ...
              <div className="rounded-xl border border-amber-100 bg-amber-50 p-2 text-amber-800">
                <p className="mb-1 text-[10px] font-bold uppercase tracking-wide">Revisión manual</p>
              ...
              <div className="rounded-xl border border-red-100 bg-red-50 p-2 text-red-800">
                <p className="mb-1 text-[10px] font-bold uppercase tracking-wide">Rechazo auto.</p>
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => openEdit(p)} className="flex-1 rounded-2xl border border-slate-200 py-2 text-xs font-semibold text-slate-600">Editar</button>
              {p.is_active ? (
                <button onClick={() => deactivate(p._id)} className="flex-1 rounded-2xl border border-red-100 py-2 text-xs font-semibold text-red-600">Desactivar</button>
              ) : (
                <button onClick={() => activate(p._id)} className="flex-1 rounded-2xl border border-green-100 py-2 text-xs font-semibold text-green-700">Activar</button>
              )}
            </div>
          </div>
        ))}
