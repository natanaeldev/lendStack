    ...
    <>
      <header className="relative overflow-hidden"
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3">
          ...
          {/* ── Quick-pay button (centre, always visible when logged in) ── */}
          {session?.user && (
            <button
              onClick={() => setShowPayment(true)}
              title="Registrar pago de cuota"
              className="w-9 h-9 rounded-full flex items-center justify-center text-white font-black text-xl transition-all hover:scale-110 active:scale-95 shadow-lg"
              style={{ background: 'linear-gradient(135deg,#1565C0,#0D2B5E)', boxShadow: '0 2px 12px rgba(21,101,192,.5)' }}>
              +
            </button>
          )}
