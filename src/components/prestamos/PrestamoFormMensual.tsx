  ...
  return (
    <div className="space-y-4">
      ...
      <Section title="Tasa y evaluacion" subtitle="Usa el perfil de riesgo o fija una tasa manual si aplica.">
        ...
        {rateMode === 'monthly' && (
          <Field label="Interes mensual (%)" hint="Ingresa la tasa mensual exacta para este caso.">
      ...
      <Section title="Inicio y notas" subtitle="Deja la fecha lista y agrega contexto operativo solo si hace falta.">
        ...
        <Field label="Observaciones" hint="Notas para el equipo, la ruta o seguimiento.">
          <textarea value={notes} onChange={(event) => onChange({ notes: event.target.value })} rows={4} placeholder="Ej: cobrar despues de las 4pm o validar garante" className={`${inputClassName} min-h-[120px] resize-y`} />
        </Field>
      </Section>
    </div>
  )
