...
export default function PrestamoFormSemanal({
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <div>
        <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-500">Plazo (semanas)</label>
      ...
      <div>
        ...
        <input value="Semanal" readOnly className={`${inputClassName} bg-slate-50`} />
      </div>
      <div>
      ...
      <div className="sm:col-span-2">
        ...
        <textarea value={notes} onChange={(event) => onChange({ notes: event.target.value })} rows={3} placeholder="Notas internas para ruta, cobranza o seguimiento" className={`${inputClassName} min-h-[96px] resize-y`} />
      </div>
    </div>
  )
}
