...
export default function LoansPanel({ onViewLoan, createRequestKey = 0 }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(data.error ?? 'Error al cargar prestamos')
        setLoans(data.loans ?? [])
      })
      .catch((loadError: any) => setError(loadError?.message ?? 'Error al cargar prestamos'))
  ...
  const filtered = useMemo(() => {
    ...
    return loans.filter((loan) =>
      ...
      loan._id.includes(q) ||
      loan.borrowerPhone.includes(q),
    )
  }, [loans, search])

  const metrics = useMemo(() => {
    const active = loans.filter((loan) => ['active', 'delinquent'].includes(loan.status))
    const delinquent = loans.filter((loan) => loan.status === 'delinquent')
    return {
    ...
    <>
      <div className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-slate-800">Prestamos</h2>
            <p className="text-xs sm:text-sm text-slate-500">Ciclo de vida operacional de la cartera y alta rapida desde esta misma vista.</p>
          </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-4 sm:p-5" style={{ boxShadow: '0 8px 24px rgba(15,23,42,.05)' }}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h3 className="mt-1 text-base font-bold text-slate-800">Crear prestamo sin pasar por Clientes</h3>
              <p className="mt-1 text-sm text-slate-500">Elige tipo, cliente y condiciones en un drawer optimizado para movil y escritorio.</p>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
          {[
            { label: 'Cartera activa', value: formatCurrency(metrics.activePortfolio, metrics.currency), sub: `${metrics.totalActive} prestamos`, dot: '#10B981', bg: '#ECFDF5' },
            ...
            { label: 'Total prestamos', value: String(loans.length), sub: 'en el sistema', dot: '#3B82F6', bg: '#EFF6FF' },
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          <input type="text" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre, ID, telefono..." className="w-full sm:flex-1 px-4 py-2.5 rounded-xl border-2 border-slate-200 text-sm focus:outline-none focus:border-blue-500 bg-white" />
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="w-full sm:w-auto px-3 py-2.5 rounded-xl border-2 border-slate-200 text-sm focus:outline-none focus:border-blue-500 bg-white">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-slate-400 rounded-3xl border border-dashed border-slate-200 bg-white">
            ...
            <p className="font-semibold text-slate-600">No se encontraron prestamos</p>
            <p className="text-sm">Ajusta filtros o crea uno nuevo sin salir de Prestamos.</p>
                    ...
                    {filtered.map((loan) => (
                      <tr key={loan._id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer" style={loan.status === 'delinquent' ? { background: '#FFFBF5' } : {}} onClick={() => onViewLoan(loan._id)}>
                        ...
                        <td className="px-3 py-3 text-center">
                          <button onClick={(event) => { event.stopPropagation(); onViewLoan(loan._id) }} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-blue-600 hover:bg-blue-50 transition-colors">
                            Ver ?
                          </button>
                        </td>
                      </tr>
