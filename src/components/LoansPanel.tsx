...
const LOAN_TYPE_LABELS: Record<string, string> = {
  ...
  weekly: 'Semanal',
  carrito: 'Inter?s plano',
  flat: 'Inter?s plano',
  flat_rate: 'Inter?s plano',
  interes_plano: 'Inter?s plano',
}
...
function loanTypeLabel(loan: LoanRow) {
  ...
  if (normalized === 'amortized' && hasFlatSignals) return 'Inter?s plano'
...
export default function LoansPanel({ onViewLoan, createRequestKey = 0 }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .catch(() => setError('Error al cargar pr?stamos'))
    ...
    <div>
      <div className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-slate-800">Pr?stamos</h2>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-4 sm:p-5" style={{ boxShadow: '0 8px 24px rgba(15,23,42,.05)' }}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h3 className="mt-1 text-base font-bold text-slate-800">Crear pr?stamo sin pasar por Clientes</h3>
              <p className="mt-1 text-sm text-slate-500">Eleg? tipo, cliente y condiciones sin salir de la vista de Pr?stamos.</p>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
          {[
            { label: 'Cartera activa', value: formatCurrency(metrics.activePortfolio, metrics.currency), sub: `${metrics.totalActive} pr?stamos`, dot: '#10B981', bg: '#ECFDF5' },
            ...
            { label: 'Total pr?stamos', value: String(loans.length), sub: 'en el sistema', dot: '#3B82F6', bg: '#EFF6FF' },
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          <input
            ...
            placeholder="Buscar por nombre, ID, tel?fono?"
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-slate-400 rounded-3xl border border-dashed border-slate-200 bg-white">
            ...
            <p className="font-semibold text-slate-600">No se encontraron pr?stamos</p>
            <p className="text-sm">Prob? otro filtro o cre? un pr?stamo nuevo sin salir de esta vista.</p>
