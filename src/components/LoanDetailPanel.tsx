...
const inputCls = 'w-full px-3 py-2 rounded-xl border-2 border-slate-200 text-sm focus:outline-none focus:border-blue-500 transition-colors bg-white'
  ...
  return (
    <div className="flex items-center gap-0 overflow-x-auto pb-1">
          ...
          <div key={step} className="flex items-center flex-shrink-0">
            <div className="flex flex-col items-center gap-1" style={{ minWidth: 72 }}>
              ...
              <span className="text-[10px] font-semibold text-center leading-tight"
                style={{ color: done || now ? cfg.color : '#9CA3AF', maxWidth: 64 }}>
                {cfg.label}
              </span>
            </div>
            {idx < LIFECYCLE_STEPS.length - 1 && (
              <div className="h-0.5 w-8 mx-0.5 rounded"
      ...
      {(terminalOk || terminalBad || isDelinquent) && (
        <>
          <div className="h-0.5 w-8 mx-0.5 rounded" style={{ background: '#E5E7EB' }} />
          <div className="flex flex-col items-center gap-1" style={{ minWidth: 72 }}>
            ...
            <span className="text-[10px] font-semibold text-center leading-tight"
              style={{ color: terminalOk ? '#0284C7' : isDelinquent ? '#F97316' : '#B91C1C', maxWidth: 64 }}>
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border-2 border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50">
            Cancelar
          </button>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        ...
        <div className="space-y-4">
          ...
          {unpaidInstallments.length > 0 && (
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">
                Cuota específica (opcional — por defecto la más antigua)
              </label>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border-2 border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50">Cancelar</button>
          <button onClick={submit} disabled={loading}
            className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white disabled:opacity-50"
...
function CollectionModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        <h3 className="text-lg font-bold text-slate-800 mb-4">Registrar acción de cobranza</h3>
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Fecha</label>
            <input type="date" value={date} onChange={e => setDate(e.target.value)} className={inputCls} />
...
export default function LoanDetailPanel({ loanId, onBack, onViewBorrower }: Props) {
  ...
  const cfg        = LOAN_STATUS_CONFIG[loan.status] ?? LOAN_STATUS_CONFIG.application_submitted
        ...
        <div className="rounded-xl px-5 py-3 flex items-center gap-3"
          ...
          <div>
            <p className="text-sm font-bold" style={{ color: '#7C2D12' }}>
              Préstamo moroso — {delinquency.daysPastDue} días de atraso
            </p>
